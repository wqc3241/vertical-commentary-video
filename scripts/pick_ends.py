...
scenes.py may guide this:
  - a scene with "lock": True keeps its hand-set tin (use for user-specified exact windows)
  - END_OVERRIDES = {"S1": 168.5}   -> force that scene to end exactly there
...
for sc in SCENES:
    ...
    used[reel].append(E); proposed[sid] = round(E-D, 2)
    print(f"{sid} {reel:20s} D={D:5.1f}  end->{E:6.1f}  tin {sc['tin']:6.1f} -> {proposed[sid]:6.1f}")
...
for i,sc in enumerate(SCENES):
    ...
    t = tin+D*0.5 if sc["treat"].startswith("card") else tin+D-0.3
    o = f"/tmp/ve_{i}.jpg"
...
print("\n-> build/proposed_tins.json + build/verify_ends.png (inspect: no opponent-celebration/replay endings)")
