...
if __name__=="__main__":
    ...
    for sc in SCENES:
        ...
        b=badgaps(os.path.join(AUD,sid+".wav"),CAPTIONS[sid])
        if b: flagged[sid]=b
        print(sid,"BAD" if b else "ok",b,flush=True)
    json.dump(flagged,open(os.path.join(BUILD,"pause_audit.json"),"w"),ensure_ascii=False)
    if mode=="fix" and flagged:
        for sc in SCENES:
            sid=sc["id"]
            if sid not in flagged: continue
            best=(len(flagged[sid]), os.path.join(AUD,sid+".wav"))
            for t in range(1,5):
                ...
                b=badgaps(fin,CAPTIONS[sid])
                print(f"{sid} try{t}: bad={len(b)}",b,flush=True)
                if len(b)<best[0]: best=(len(b),fin)
                if len(b)==0: break
            if best[1]!=os.path.join(AUD,sid+".wav"): shutil.copy(best[1],os.path.join(AUD,sid+".wav"))
            print(f"{sid}: PICKED bad={best[0]} src={os.path.basename(best[1])}",flush=True)
        print("FIX DONE")
